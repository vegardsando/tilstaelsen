...
class LabelEnvironmentPlugin extends BasePlugin
{
    ...
    function init() {
        ...
        if ( craft()->request->isCpRequest() && (craft()->userSession->isAdmin() || craft()->config->get('devMode')))
        {
            // the includeJs method lets us add js to the bottom of the page
            craft()->templates->includeHeadHtml('<script>window.environmentName ="' . $environmentName .'";</script>');
			...
			craft()->templates->includeJsResource('labelenvironment/favicon.js');
